...
- **Session-scope**(`cron` skill → `CronCreate`)—— agent 重启就死。适合一次性提醒或探针。
- **HEARTBEAT.md** —— 每次 heartbeat 触发时让 agent 懒检查一下。跨重启存活。适合需要 session 上下文的任务。
- **LaunchAgent plist**(或 `crontab`)—— OS 级,Mac 不重启就不死。适合监控、日志采集、漏不得的事。

完整走一遍见 [`docs/cron.zh-CN.md`](docs/cron.zh-CN.md)。示例 LaunchAgent plist 在生成的 agent 里 `launchd/cron-example.plist.tmpl` —— 拷 + 改 + `launchctl load`。
