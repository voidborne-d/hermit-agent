 ...
 *
 * 用于 connectOverCDP 场景：自启动 Chrome + Playwright CDP 遥控
...
async function verifyStealthStatus(page) {
  return await page.evaluate(() => ({
    ...
    playwrightGlobals: ['__playwright', '__pw_manual', '__PW_inspect'].filter(k => k in window),
    cdcKeys: Object.keys(window).filter(k => k.startsWith('cdc_')),
  }));
}

module.exports = { applyStealthToPage, applyStealthToContext, verifyStealthStatus, STEALTH_INIT_SCRIPT };
